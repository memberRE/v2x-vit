...
from v2xvit.models.pointnet_util import index_points
...
def spare_point_cloud_weighted(lidar, veh_mask, npoint):
    xyzs = lidar[..., :3]
    veh_mask = torch.from_numpy(veh_mask[None, ...].astype(np.float32)).cuda()
    fps_idx = furthest_point_sample_weights(xyzs.contiguous(), veh_mask, npoint).long()
    torch.cuda.empty_cache()
    new_lidar = index_points(lidar, fps_idx)
    torch.cuda.empty_cache()
    return new_lidar
