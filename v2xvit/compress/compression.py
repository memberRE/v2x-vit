...
class CompressTools:
    ...
    def __call__(self, lidar):
        ...
        # normalize xyzs
        # shift = np.mean(lidar_xyz, axis=0)  # 1 x 3
        # lidar_xyz -= shift
        # max_coord, min_coord = np.max(lidar_xyz), np.min(lidar_xyz)
        # lidar_xyz = lidar_xyz - min_coord
        # lidar_xyz = lidar_xyz / (max_coord - min_coord)
        # lidar_xyz = torch.from_numpy(lidar_xyz).float().cuda()
        # map_xyzs = lidar_xyz * (self.mapsize if self.use_patch else self.range)
        # xyzs = np.floor(map_xyzs).astype('float32')
        ...
        for patch_idx in cubes.keys():
            ...
            pred_patches = pred_patches * (meta_data['max_coord'] - meta_data['min_coord']) + meta_data['min_coord'] + meta_data['shift']
